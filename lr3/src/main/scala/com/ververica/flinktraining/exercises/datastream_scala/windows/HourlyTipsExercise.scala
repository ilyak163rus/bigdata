...
object HourlyTipsExercise {
  ...
  class AddTips extends ProcessWindowFunction[TaxiFare, (Long, Long, Float), Long, TimeWindow] {

    override def process(
        driverId: Long,
        context: Context,
        fares: Iterable[TaxiFare],
        out: Collector[(Long, Long, Float)]
    ): Unit = {

      val totalTips = fares.map(_.tip).sum

      out.collect((context.window.getEnd, driverId, totalTips))
    }
  }
